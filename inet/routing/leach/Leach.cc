...
void Leach::start() {
    ...
    auto* ipv4Data = const_cast<Ipv4InterfaceData*>(interface80211ptr->getProtocolData<Ipv4InterfaceData>());
    CHK(ipv4Data)->joinMulticastGroup(Ipv4Address::LL_MANET_ROUTERS);
...
void Leach::handleSelfMessage(cMessage *msg) {
    if (msg == event) {
        if (event->getKind() == SELF) {
            ...
            send(packet, "ipOut");
            addToEventLog(source, Ipv4Address(255, 255, 255, 255), "CTRL",
                    "SENT");
            controlPktSent += 1;
            packet = nullptr;
            ctrlPkt = nullptr;
...
string Leach::resolveFingerprint(Ipv4Address nodeAddr, Ipv4Address CHAddr) {
    ...
    hash<string> hashFn;
    size_t hashResolved = hashFn(
            CHAddrResolved + nodeAddrResolved + simTimeResolved);
...
void Leach::addToEventLog(Ipv4Address srcAddr, Ipv4Address destAddr,
        string packet, string type) {
    const char *srcNodeName =
            L3AddressResolver().findHostWithAddress(srcAddr)->getFullName();
    const char *destNodeName;
    if (destAddr.isLimitedBroadcastAddress()) {
        destNodeName = "Broadcast";
    } else {
        destNodeName =
                L3AddressResolver().findHostWithAddress(destAddr)->getFullName();
    }

    ICcEnergyStorage *energyStorageModule = check_and_cast<ICcEnergyStorage*>(
...
void Leach::addToNodePosList() {
    IMobility *mobilityModule = check_and_cast<IMobility*>(
...
//        }
    ...
    for (auto &it : eventLog) {
        string resolvedResidualCapacity = it.residualCapacity.str().erase(
                it.residualCapacity.str().size() - 2, 2); // remove the C unit from end of string
        if (it.type == "SENT") {
            eventLogFile
                    << to_string(it.time) + "," + it.srcNodeName + ","
                            + it.destNodeName + "," + it.packet + "," + it.type
                            + "," + resolvedResidualCapacity << std::endl;
        } else {
            eventLogFile
                    << to_string(it.time) + "," + it.destNodeName + ","
                            + it.srcNodeName + "," + it.packet + "," + it.type
                            + "," + resolvedResidualCapacity << std::endl;
        }
    }
    eventLogFile.close();
    ...
    for (auto &positionIterator : nodePositionList) {
        for (auto &weightIterator : nodeWeightList) {
            if (positionIterator.nodeName == weightIterator.nodeName) {
                nodePosFile
                        << positionIterator.nodeName + ","
                                + to_string(positionIterator.posX) + ","
                                + to_string(positionIterator.posY) + ","
...
void Leach::finish() {
    ...
    EV << "Total control packets sent by CH:                    "
              << controlPktSent << endl;
    EV << "Total control packets received by NCHs from CH:      "
              << controlPktReceived << endl;
    EV << "Total data packets sent to CH:                               "
              << dataPktSent << endl;
    EV << "Total data packets received by CH from NCHs:                 "
              << dataPktReceived << endl;
    EV << "Total data packets received by CH from NCHs verified:        "
              << dataPktReceivedVerf << endl;
    EV << "Total BS packets sent by CH:                         " << bsPktSent
              << endl;
